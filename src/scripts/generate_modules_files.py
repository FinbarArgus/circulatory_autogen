...
Inputs: CellML model and the output directory where files are created
"""
import argparse
import json
import os.path
...
def _generate_module_config(output_dir, variables, constants, file_prefix, component_name):
    config = {
    ...
    file_path = os.path.join(output_dir, f"{file_prefix}_module_config.json")

    with open(file_path, "w") as fh:
        json.dump(module_config, fh, indent=2)

# Generate parameters.csv file
def _generate_parameters_csv(output_dir, constants, file_prefix, data_reference):
    ...
    with open(file_path, "w") as fh:
        ...
        for constant in constants:
            fh.writelines([f"{constant[0]}_{file_prefix},{constant[1]},{constant[3]},{data_reference}\n"])

# Generate CellML module
def _generate_cellml_module(output_dir, input_model, states, file_prefix):
    ...
    file_path = os.path.join(output_dir, f"{file_prefix}_modules.xml")
...
def main():
    ...
    # Define file_prefix and data_reference
    file_prefix = "NKE_pump"
    data_reference = "user_defined"

    _generate_module_config(args.output_dir, variables, constants, file_prefix, component.name())

    _generate_parameters_csv(args.output_dir, constants, file_prefix, data_reference)

    _generate_cellml_module(args.output_dir, args.input_model, states, file_prefix)
