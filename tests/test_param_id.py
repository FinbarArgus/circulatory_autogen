...
def test_SN_differentiable_obs_data_uses_only_casadi_ops(resources_dir):
    """SN_differentiable_obs_data.json must use only @differentiable operations and costs."""
    from param_id.differentiable import assert_casadi_differentiable
    from parsers.PrimitiveParsers import scriptFunctionParser

    obs_path = os.path.join(resources_dir, "SN_differentiable_obs_data.json")
    assert os.path.isfile(obs_path), f"Missing obs data: {obs_path}"
    ...
    with open(obs_path, encoding="utf-8") as handle:
        obs_bundle = json.load(handle)

    operations = [item.get("operation") for item in obs_bundle.get("data_items", [])]
    cost_types = [item.get("cost_type") for item in obs_bundle.get("data_items", [])]
    obs_info = {"operations": operations}

    assert_casadi_differentiable(obs_info, cost_types, ops, costs)


@pytest.mark.integration
@pytest.mark.slow
@pytest.mark.mpi
def test_param_id_SN_simple_casadi_differentiable(
    base_user_inputs, resources_dir, temp_output_dir, temp_generated_models_dir, mpi_comm
):
    """
    CasADi sp_minimize on SN_simple using only @differentiable observables.

    Compared with 3compartment, this isolates whether CVODES adjoint failures are due to
    model stiffness/size versus CasADi/solver infrastructure. SN_simple is smaller but
    still has piecewise gating; obs use max/mean/steady_state_* only (no spike finders).
    """
    rank = mpi_comm.Get_rank()
    ...
    config.update({
        ...
        "param_id_obs_path": os.path.join(resources_dir, "SN_differentiable_obs_data.json"),
        "param_id_output_dir": temp_output_dir,
        "generated_models_dir": temp_generated_models_dir,
        "optimiser_options": {
    ...
    if rank == 0:
        success = generate_with_new_architecture(False, config)
        assert success, "CasADi Python model generation should succeed for SN_simple"
    ...
    if rank == 0:
        output_dir = os.path.join(
            temp_output_dir,
            f"{config['param_id_method']}_SN_simple_SN_differentiable_obs_data",
        )
        assert os.path.exists(output_dir), f"Output directory should exist: {output_dir}"
